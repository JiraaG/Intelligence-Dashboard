...
> 15. **API Phase 5+:** day view via `GET /api/map-summary`; relations via `GET /api/map-relations` (archi MapLibre = macro multicolore solida a tutti gli zoom; legacy Leaflet `relationsPane` = dash+fan ≥5; click → carosello bilaterale); saved vault via `GET /api/saved-summary` (no date); nation open via `GET /api/articles` con envelope `{ items, next_cursor, total }` (page ≤ 100; FE concatena); saved open via `?saved=true` (ignora date). Mock solo con `MOCK_MODE`.
