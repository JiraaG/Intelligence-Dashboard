...
> 9. **Hatching day-view (zoom &lt; 5):** path **MapLibre** = fasce soft O→E (1 colore × tipologia da `map-summary`; mainland US/RU / largest-polygon; helper `maplibre/country-category-fills.ts`) — **non** `fill-pattern` barcode. Path **Leaflet** legacy = SVG combo pattern. Click nazione = `pickCountryCodeAt`.
