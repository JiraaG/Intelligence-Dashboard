...
> 3. **Hub disco + fan emoji**: nation open usa hub compatto (`radar-spider-root`) e spiderfy emoji della categoria attiva (max 24 icone; size/distanza adattivi). Il root non deve sparire al cambio categoria (`unspiderfied` no-op se `restoreDetailHubOnUnspiderfy === false`).
