...
Quadro operativo aggiornato **2026-07-27** (FONTI feed management **COMPLETE / GATE VERDE**; FinOps UI Metrics **COMPLETE / GATE VERDE**; Eager drain worker **COMPLETE / GATE VERDE**; FinOps LLM Wave A **COMPLETE / GATE VERDE (condizionato)**; Fase C semantic dedup **GATE VERDE**; Metrics 013 **COMPLETE / GATE VERDE**; MapLibre hatching isole/anti-bleed **GATE VERDE**; Wave 1 Relazioni nazioni **GATE VERDE**; Fase A LLM locale **COMPLETE**; Fix STATUS real-time SSE **COMPLETE** 2026-07-25).  
...
| Area | Dove | Note |
|------|------|------|
| FONTI — attività giorno + catalogo feed | [`complete/plan_impl_fonti_feed_management.md`](complete/plan_impl_fonti_feed_management.md) | **COMPLETE / GATE VERDE** 2026-07-27 — topbar FONTI (Giorno `published_at` + Catalogo toggle Miniflux); STATUS Sorgenti N/M; `GET/PATCH /api/feeds`; by-feed `date_field`; ops `enabled`. Walkthrough: [`complete/walkthrough_fonti_feed_management.md`](complete/walkthrough_fonti_feed_management.md). Prompt: [`prompts/done/agent_prompt_fonti_feed_management.md`](prompts/done/agent_prompt_fonti_feed_management.md). |
| Distinzione BORDERLINE in Carosello & Migration 017 | [`radar/backend/app/classification/client.py`](../radar/backend/app/classification/client.py) | **COMPLETE / GATE VERDE** 2026-07-25 — Valorizzazione `classification_lane = 'borderline'` per articoli in fascia euristica BORDERLINE (senza escalation); migrazione DB `017_borderline_classification_lane.sql` applicata retroattivamente; visualizzazione dinamica nel carosello FinOps sidebar (`deepseek-v4-flash (borderline)`). |
| Fix STATUS real-time SSE (startup retries + safety net) | [`radar/frontend/src/app/services/state.service.ts`](../radar/frontend/src/app/services/state.service.ts) | **COMPLETE / GATE VERDE** 2026-07-25 — Fix A (reload all'apertura SSE `open`), Fix B (retries condizionali 15s/45s su `degraded`), Fix C (safety net 5min solo se `degraded`). Zero overhead a regime nominale. |
| FinOps UI — card + STATUS/COSTI topbar (Fase 013 UI + Audit + split) | [`complete/plan_impl_finops_ui_metrics.md`](complete/plan_impl_finops_ui_metrics.md) | **COMPLETE / GATE VERDE** 2026-07-24 — W0–W7 + closeout + Audit. **Post-GATE:** split topbar **STATUS sinistra** / **COSTI destra** (`5b84a34`+); day_end RPD + BORDERLINE + Gestione & Effort (`ece7bf5`); effort ledger `015` + breakdown `(model, reasoning_effort)`. SoT UI: [`docs/03_frontend_and_ui.md`](../docs/03_frontend_and_ui.md). Walkthrough storico hub aggregato: [`complete/walkthrough_finops_ui_metrics.md`](complete/walkthrough_finops_ui_metrics.md). Audit: [`complete/audit_finops_status_costs_numbers.md`](complete/audit_finops_status_costs_numbers.md). Prompt split: [`prompts/done/agent_prompt_split_status_costi_topbar.md`](prompts/done/agent_prompt_split_status_costi_topbar.md) (SUPERSEDED layout). |
| Eager drain all’avvio worker (Miniflux) | [`complete/plan_impl_eager_drain_startup.md`](complete/plan_impl_eager_drain_startup.md) | **COMPLETE / GATE VERDE** 2026-07-24 — drain-until-empty + settle 15s + guard `success==0`; pytest 13/13; live verify PASS (3 batch 50→50→6 senza wait 900s inter-batch). Prompt: [`prompts/done/agent_prompt_eager_drain_startup.md`](prompts/done/agent_prompt_eager_drain_startup.md), closeout [`prompts/done/agent_prompt_eager_drain_closeout.md`](prompts/done/agent_prompt_eager_drain_closeout.md). |
