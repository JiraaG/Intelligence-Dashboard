...
- **Contatori per-modello (Profilo A / Gemini):** I contatori RPM/TPM/RPD nel ledger sono **per modello** (pool separati). Ciascun modello della catena `LLM_SIMPLE.models` eredita il default di lane `LLM_SIMPLE_*` (o override CSV `LLM_SIMPLE_MODEL_LIMITS`). Se `gemini-3.1-flash-lite` esaurisce i suoi 500 RPD, `gemini-3.5-flash-lite` con residuo rimane liberamente utilizzabile senza dover alzare `LLM_SIMPLE_RPD`. Soft-trim worker iberna il ciclo solo se **tutti** i modelli gestiti della catena SIMPLE sono esausti e non vi è residual COMPLEX.
