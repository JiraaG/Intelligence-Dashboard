...
- **Archi relazioni (Fase H + UI):** MapLibre great-circle (default) = **sempre** una linea aggregata multicolore continua; legacy Leaflet `relationsPane` z 550 = zoom &lt; 4 multicolore / zoom ≥ 4 per-categoria + geometric dash; click → `relationClicked` / `loadRelationArticles` (bilaterale). **Filtro nazioni Wave 1 (DONE):** toolbar **RELAZIONI ATTIVE** (default OFF, toggle iOS) → `visibleMapRelations` (OR stella); paint/hover invariati. **Sentiment / Tipologia / Relazioni:** stesso tooltip (un bottone Seleziona↔Deseleziona tutto, filtro testo, toggle iOS a destra); filtro nazione anche su LETTE/TROVATE e SALVATE. Piano: `plan-audit/complete/plan_impl_map_relations_nation_filter.md`
