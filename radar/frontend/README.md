...
- **FinOps topbar (STATUS / COSTI):** `.toolbar-left` inizia con **STATUS** (live `GET /api/metrics/status` — RPD, borderline, Gestione & Effort); `.toolbar-right` termina con **COSTI** (giorno `GET /api/metrics/summary` — 6 sezioni + `overall`). MOCK_MODE: `article-mock.service` deve esporsi `getMetricsSummary` / `getMetricsStatus` allineati al contratto (incl. `borderline`, `reasoning_effort`). Dettaglio: [`docs/03_frontend_and_ui.md`](../../docs/03_frontend_and_ui.md) + skill `radar-api-contract` / `spatial-data-mocking` Test 8.
