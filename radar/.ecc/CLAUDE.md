...
- **Phase B (Real-Time Ingestion & Soft Refresh) DONE / GATE VERDE**: Webhook HMAC (`POST /api/webhooks/miniflux`), streaming SSE (`GET /api/articles/events`), dedicated Postgres LISTEN connections (no pool), Angular zone-isolated soft refresh with reference-preserving merge, pending mutation protection, ed **estensione resilienza SSE STATUS Real-Time** (Fix A reload open, Fix B retries 15s/45s su `degraded`, Fix C safety net 5m).
...
- **FinOps Wave A COMPLETE / GATE VERDE (condizionato)** (2026-07-22; soak OK): M1–M6 token saving/caching; SoT `plan-audit/complete/plan_impl_llm_finops_token_caching.md` + twin `plan-audit/complete/plan_impl_llm_finops_token_caching_verification.md`; migrations `013`+`014`.
...
- **FONTI feed management COMPLETE / GATE VERDE** (2026-07-27): topbar **FONTI** (Giorno `published_at` + Catalogo toggle Miniflux); STATUS snippet Sorgenti N/M; `GET/PATCH /api/feeds`; by-feed `date_field`. SoT: `plan-audit/complete/plan_impl_fonti_feed_management.md` + walkthrough twin.
- **Miniflux DNS harden** (2026-07-27): Compose `dns:` su `radar-miniflux`; worker gate `WORKER_DNS_READY_*` prima di `refresh_all_feeds`; ops `verify_miniflux_egress.py` (per_feed). Evita mass ERR sticky `no such host`.
- **Presenti (Phase 1–5 + follow-up):** migrazioni `001`–`017` (incluso `008_outbox_miniflux_marked_at`, `009_llm_model_cooldown`, `010_articles_is_saved`, `011_articles_related_countries`, `012_pgvector_article_embeddings`, `013_metrics_and_feed_tracking`, `014_articles_content_sha256`, `015_llm_ledger_reasoning_effort`, `016_add_new_categories` = **15 categorie**, `017_borderline_classification_lane`), outbox, ledger quote, cooldown modelli (RPD → `day_end`), `radar-worker`, reti `radar-edge`/`radar-data`, `/health/live`+`/ready`, CSP Nginx, `ops/` backup, Gemini `build_gemini_response_schema()`, FE `MOCK_MODE` / DestroyRef / XSS-safe markers / read-unread senza rebuild cluster / **`detailError` nation-fetch → banner toolbar (T-P1-04)** / **Notizie Salvate** (`is_saved`, saved-summary, toolbar vault) / **FinOps topbar STATUS (left) → FONTI → calendario; COSTI (right)** / **Resilienza connessione SSE STATUS (startup retries + safety net)**.
- **Phase 5 API/FE:** `GET /api/map-summary` (`country×category`); `GET /api/map-relations` (archi MapLibre great-circle default; FE **RELAZIONI ATTIVE** → `visibleMapRelations` default OFF; legacy Leaflet `relationsPane` z550; hover/click → `loadRelationArticles`); `GET /api/saved-summary` (vault, no date); `GET /api/articles` → `{items,next_cursor,total}` (keyset `id`, limit≤100, LATERAL; `saved=true` cross-day); `PATCH read_status` / `saved_status` (unread⇒unsave; save⇒read); metrics FinOps + **FONTI** `GET /api/feeds` / `PATCH /api/feeds/{id}/toggle` / by-feed `date_field`; `backend/app/api/articles_query.py` + `feeds.py`; migrazioni `007`+. FE: giorno da summary + **pin nazione** sul centroide paese (`getCountryCentroid`; US/RU mainland — non media lat/lng pezzo); hatching click latch hatching via `pickCountryCodeAt`; nazione = hub disco + spiderfy categoria attiva; vault salvati = tooltip + spiderfy parity LETTE/TROVATE. **Vietato** `article-list`. Sidebar freeze resta (due eccezioni mirate: toggle Salva e chip `related_countries`). Restore tip map/summary: `a240b3c`; archi solidi MapLibre: `0d942ed`; W1 filtri nazioni: `ec771b1`.
...
│   │   ├── 013_metrics_and_feed_tracking.sql
│   │   ├── 014_articles_content_sha256.sql
│   │   ├── 015_llm_ledger_reasoning_effort.sql
│   │   ├── 016_add_new_categories.sql
│   │   └── 017_borderline_classification_lane.sql
│   └── app/
│       ├── __init__.py
...
Miniflux API (`WORKER_POLL_INTERVAL_SECONDS`, default 900)
  ...
  Validate entry → sanitize HTML → dedup URL → content hash (M6 lookback 24h)
        ...
        ▼
  Embed + semantic dedup (pgvector) → M5 high-sim direct bypass OR optional quality:compare (COMPLEX)
