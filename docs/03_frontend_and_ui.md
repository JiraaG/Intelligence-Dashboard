...
1. `GET /api/map-summary?date=…` → hatching per paese + a zoom ≥ 5 **pallini numerati** (`country × category`, count)
...
3. Marker articolo solo per il paese aperto
4. Spiderfy allineato alla **categoria attiva** (pallino / pill / slide carosello). Stessa categoria allo scroll → solo highlight (`lastSpiderfyKey`), niente collapse/reopen

**Close / cambio paese:** clear detail markers; tornano i pallini summary.
...
Un `markerClusterGroup` **per ciascuna delle 10 categorie**:
