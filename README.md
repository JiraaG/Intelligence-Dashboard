...
**Routing LLM:** `.env.example` ops tipico = **Profilo B** + `LLM_ROUTING_MODE=complexity`. Effort BORDERLINE configurabile via `LLM_BORDERLINE_REASONING_EFFORT` (default/ops tipico `high`; COMPLEX tipico `max` su `deepseek-v4-flash`; escalate ValidationError → primary COMPLEX). quality:compare usa effort `high`. Default codice boot-safe (senza env) = `LLM_ROUTING_MODE=off` + `LLM_ROUTING_SHADOW=true` — non confondere i due. Local-Hybrid = **Profilo F** (SIMPLE Ollama / COMPLEX cloud).
...
| 03 | [docs/03_frontend_and_ui.md](docs/03_frontend_and_ui.md) | Mappa MapLibre (hatching isole/anti-bleed, pin, spiderfy), Legenda Tipologie Popover Grid (A-Z, conteggi notizie, glow illuminazione mappa senza offuscamento, auto-close su zoom), selettore Tipologia toolbar, map-summary, archi relazioni (hover/click bilaterale), `MOCK_MODE`, stato UI |
...
| Fase G — Obsidian wiki-links | `feature/upgrades` (2026-07-27) | `[[wiki-link]]` + hub `_meta/` Radar→Vault; Graph hub-centric (`obsidian_graph.py`, `radar/obsidian/graph.json`) — **GATE VERDE** |
...
| Vault Obsidian wiki-link (Fase G) | `radar/backend/app/commit/{factory,wikilinks,hubs,obsidian_graph,router,outbox}.py`, `radar/obsidian/graph.json` |
